...
public class WordleGUI extends Application {
	...
	public void start(Stage primaryStage) {
		...
		layoutWindow();
		// WordleTimer wordleTimer = new WordleTimer(60); // Set the initial time to 60
		// seconds.
	...
	// Initializes the user interface components, setting up event listeners and
	// layout
	private void layoutWindow() {
		root = new AnchorPane();

		// Label timerLabel = new Label();
		// timerLabel.textProperty().bind(Bindings.format("Time remaining: %d seconds",
		// wordleTimer.timeRemainingProperty()));

		VBox vbox = new VBox(20);
		vbox.setPrefSize(600, 400);
		vbox.setAlignment(Pos.CENTER);
		// vbox.getChildren().add(1, timerLabel); // Add the timerLabel right below the
		// instructionsLabel.
	...
	// Handles the submit button click, processing the user's guess and updating the
	// game state
	private void handleSubmit() {
		...
		if (wordleController.isGameOver()) {
			...
			if (wordleController.isGameWon()) {
				instructionsLabel
						.setText("Congratulations! The word was: " + correctWord + ". Press 'Submit' to play again.");
			} else {
				instructionsLabel
						.setText("No more attempts. The word was: " + correctWord + ". Press 'Submit' to play again.");
			}
	...
	// Starts a new game by resetting the game state, updating the user interface,
	// and clearing feedback
